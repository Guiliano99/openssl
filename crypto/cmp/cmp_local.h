...
# endif
    ...
    /*
     * Application-supplied request-side payload for the NonceRequest, kept
     * opaque by OpenSSL: rats_req_type is the profile type OID and
     * rats_req_info is the DER-encoded profile value carried in reqInfo
     * (ASN1_ANY).  OpenSSL never interprets these; the profile (e.g. the TPM
     * platform client) builds the value and sets it via
     * OSSL_CMP_CTX_set1_rats_reqInfo().  NULL = omit type/reqInfo.
     */
