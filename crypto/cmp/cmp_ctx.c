...
/*
 * Set the request-side nonce payload (type OID + reqInfo) for the next
 * NonceRequest, kept as an opaque profile value.  *type_oid_dot* is the
 * profile type OID in dotted form; *info_der* is the DER of the profile-
 * specific value to carry in reqInfo (ASN1_ANY) and is copied internally.
 * OpenSSL never interprets these — the profile (e.g. the TPM platform client)
 * owns their meaning and is responsible for building *info_der*.
 *
 * type_oid_dot == NULL clears both fields (the NonceRequest carries no
 * type/reqInfo).  info_der == NULL / info_der_len <= 0 keeps the type but
 * omits reqInfo (per the draft, reqInfo MUST be omitted when type is absent).
 * Returns 1 on success, 0 on error.
 */
int OSSL_CMP_CTX_set1_rats_reqInfo(OSSL_CMP_CTX *ctx, const char *type_oid_dot,
                                   ...
                                   int info_der_len)
{
    ...
    if (type_oid_dot != NULL) {
        ...
        if (info_der != NULL && info_der_len > 0
                && (info = d2i_ASN1_TYPE(NULL, &p, info_der_len)) == NULL) {
            ERR_raise(ERR_LIB_CMP, CMP_R_INVALID_ARGS);
